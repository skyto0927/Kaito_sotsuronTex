...
for i in range(4):#人
    for k in range(4):#音高
        for j in range(5):#母音
            ...
            d = []
            d.append(np.mean(cent[200:601]))
            d.append(np.mean(cent[600:1001]))
            d.append(np.mean(cent[1000:1401]))

            error = []
            error.append(np.std(cent[200:601]))
            error.append(np.std(cent[600:1001]))
            error.append(np.std(cent[1000:1401]))
            #ax.plot([1,2], d, color=color[i] , linewidth=1)

            ax.errorbar([1,2,3], d, yerr=error, elinewidth=1, color=color[j], linewidth=1, linestyle="dashed", marker="o", markersize=6, capsize=4, label=vowels[j])
